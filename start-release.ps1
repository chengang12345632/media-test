...
Write-Host "Starting All Services (Release Mode)" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
# Build projects
if (-not $SkipBuild) {
    Write-Host "[2/4] Building projects (Release mode)..." -ForegroundColor Yellow
    Write-Host "  Note: Release build takes longer time..." -ForegroundColor Yellow
    Write-Host ""
    
    Write-Host "  Building platform-server (Release)..." -ForegroundColor Gray
    ...
    Write-Host "  Building device-simulator (Release)..." -ForegroundColor Gray
...
if (-not (Test-Path $platformExe)) {
    Write-Host "  X platform-server.exe not found (Release)" -ForegroundColor Red
    Write-Host "  Please run: .\start-release.ps1" -ForegroundColor Yellow
    exit 1
}

if (-not (Test-Path $deviceExe)) {
    Write-Host "  X device-simulator.exe not found (Release)" -ForegroundColor Red
    Write-Host "  Please run: .\start-release.ps1" -ForegroundColor Yellow
    exit 1
}
...
Write-Host "  Build Mode:      Release (Optimized)" -ForegroundColor Gray
Write-Host ""
