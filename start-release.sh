...
echo "========================================"
echo "Starting All Services (Release Mode)"
echo "========================================"
echo ""
...
if [ "$1" != "--skip-build" ]; then
    echo "[2/4] Building projects (Release mode)..."
    echo "  Note: Release build takes longer time..."
    echo ""
    
    echo "  Building platform-server (Release)..."
    cd platform-server
    ...
    echo "  Building device-simulator (Release)..."
    cd device-simulator
    cargo build --release > /dev/null 2>&1
    if [ $? -ne 0 ]; then
...
PLATFORM_EXE="target/release/platform-server"
DEVICE_EXE="target/release/device-simulator"

if [ ! -f "$PLATFORM_EXE" ]; then
    echo "  X platform-server not found (Release)"
    echo "  Please run: ./start-release.sh"
    exit 1
fi

if [ ! -f "$DEVICE_EXE" ]; then
    echo "  X device-simulator not found (Release)"
    echo "  Please run: ./start-release.sh"
    exit 1
fi
...
cd "$(dirname "$0")"
RUST_LOG=info ./target/release/platform-server > /dev/null 2>&1 &
PLATFORM_PID=$!
...
cd device-simulator
RUST_LOG=info ../target/release/device-simulator --device-id device_001 --server-addr 127.0.0.1:8443 > /dev/null 2>&1 &
DEVICE_PID=$!
